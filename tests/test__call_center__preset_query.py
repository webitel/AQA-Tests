...
from utils.endpoints import CALL_CENTER, PRESET_QUERY_SERVICE
...
def test__call_center__preset_query__GET_200():
    ...
    response = request.get(endpoint=CALL_CENTER+PRESET_QUERY_SERVICE, _params={'size': 5000})
    assert response.status_code == 200
    validate_schema(instance=response.json(), schema=endpoint_schema(endpoint=CALL_CENTER+PRESET_QUERY_SERVICE,
                                                              method='GET',
                                                              status_code=200))
...
def test__call_center__preset_query_by_id__GET_200():
    request = Webitel(obf_endpoint=CALL_CENTER+PRESET_QUERY_SERVICE)
    response = request.get(endpoint=CALL_CENTER+PRESET_QUERY_SERVICE, _params={'id': PRESET_QUERY_SERVICE_ID})
    assert response.status_code == 200
    validate_schema(instance=response.json(), schema=endpoint_schema(endpoint=CALL_CENTER+PRESET_QUERY_SERVICE,
                                                              method='GET',
                                                              status_code=200,
...
def test__call_center__preset_query__POST_200():
    ...
    request = Webitel(obf_endpoint=CALL_CENTER+PRESET_QUERY_SERVICE)
    response = request.post(endpoint=CALL_CENTER+PRESET_QUERY_SERVICE, data=data)
    assert response.status_code == 200
    validate_schema(instance=response.json(), schema=endpoint_schema(endpoint=CALL_CENTER+PRESET_QUERY_SERVICE,
                                                              method='POST',
                                                              status_code=200,
...
def test__call_center__preset_query_by_id__PUT_200():
    ...
    required_fields = ['created_at', 'description', 'id', 'name', 'preset', 'section', 'updated_at']
    request = Webitel(obf_endpoint=CALL_CENTER+PRESET_QUERY_SERVICE+'/{id}')
    response = request.put(endpoint=CALL_CENTER+PRESET_QUERY_SERVICE+f'/{PRESET_QUERY_SERVICE_ID}', data=data)
    assert response.status_code == 200
    validate_schema(instance=response.json(), schema=endpoint_schema(endpoint=CALL_CENTER+PRESET_QUERY_SERVICE,
                                                              method='PUT',
                                                              status_code=200,
...
def test__call_center__preset_query_by_id__PATCH_200():
    ...
    required_fields = ['created_at', 'description', 'id', 'name', 'preset', 'section', 'updated_at']
    request = Webitel(obf_endpoint=CALL_CENTER+PRESET_QUERY_SERVICE+'/{id}')
    response = request.patch(endpoint=CALL_CENTER+PRESET_QUERY_SERVICE+f'/{PRESET_QUERY_SERVICE_ID}', data=data)
    assert response.status_code == 200
    validate_schema(instance=response.json(), schema=endpoint_schema(endpoint=CALL_CENTER+PRESET_QUERY_SERVICE,
                                                              method='PATCH',
                                                              status_code=200,
...
def test__call_center__preset_query_by_id__DELETE_200():
    ...
    request = Webitel(obf_endpoint=CALL_CENTER+PRESET_QUERY_SERVICE+'/{id}')
    response = request.delete(endpoint=CALL_CENTER+PRESET_QUERY_SERVICE+f'/{DELETE_ID}')
    assert response.status_code == 200
    validate_schema(instance=response.json(), schema=endpoint_schema(endpoint=CALL_CENTER+PRESET_QUERY_SERVICE,
                                                              method='DELETE',
                                                              status_code=200,
